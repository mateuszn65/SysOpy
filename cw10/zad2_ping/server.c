...
void handle_opponent_left(int i);
...
void handle_opponent_left(int i){
    if(clients[i]->game != NULL ){
        int player1_id = clients[i]->game->player1_id;
        int player2_id = clients[i]->game->player2_id;
        free(clients[i]->game);
...
void* ping(void * args){
    while(1){
        ...
        for(int i = 0; i < MAX_CLIENTS; i++){
            if(clients[i] == NULL)
                continue;
            sendto(clients[i]->fd, &(struct Msg){.type = PING}, sizeof(struct Msg), 0, &clients[i]->address, sizeof(struct sockaddr));
            clients[i]->active = 0;
        }
...
void handle_connect(int client_fd, struct Msg* msg, struct sockaddr* address){
    ...
    if(clients_connected % 2 == 0){
        ...
        client_waiting_id = id;
        sendto(client_fd, &(struct Msg){.type = WAITING_FOR_OPPONENT, .id = id}, sizeof(struct Msg), 0, address, sizeof(struct sockaddr));
    }
