...
void find_and_dc_client(int client_fd);
int check_username(struct Msg* msg);
int create_client(int client_fd, struct Msg* msg);
void handle_move(int client_fd, struct Msg* msg);
void handle_connect(int client_fd, struct Msg* msg);
...
void init_local(){
    if ((local_fd = socket(AF_LOCAL, SOCK_STREAM, 0)) == -1)
...
void init_net(char *port){
    if ((net_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
        error("Couldn't create socket");
    int opt = 1;
    ...
    if(listen(net_fd, MAX_CLIENTS) != 0)
        error("Listen failed");
}
...
void find_and_dc_client(int client_fd){
    for(int j = 0; j < MAX_CLIENTS; j++){
        if(clients[j] != NULL && clients[j]->fd == client_fd){
            if(clients[j]->game != NULL){
                ...
                if(player1_id == j){
                    write(clients[player2_id]->fd, &(struct Msg){.type = OPPONENT_LEFT}, sizeof(struct Msg));
                }else{
                    write(clients[player1_id]->fd, &(struct Msg){.type = OPPONENT_LEFT}, sizeof(struct Msg));
                }
...
void wait_for_clients(){
    ...
    while(1){
        ...
        for(int i = 0; i < nfds; i++){
            //printf("event fd: %d\n", events[i].data.fd);
            if (events[i].data.fd == net_fd || events[i].data.fd == local_fd){
                ...
                printf("Client's shutdown detected\n");
                find_and_dc_client(events[i].data.fd);

            }else {
                int client_fd = events[i].data.fd;
                read(client_fd, &msg, sizeof(struct Msg));

                if (msg.type == DISCONNECT){
                    find_and_dc_client(client_fd);

                }else if (msg.type == CONNECT){
                    handle_connect(client_fd, &msg);
...
void start_game(int player1_id, int player2_id){
    ...
    msg.type = START1;
    msg.id = player1_id;
    write(clients[player1_id]->fd, &msg, sizeof(struct Msg));
    msg.type = START2;
    msg.id = player2_id;
    write(clients[player2_id]->fd, &msg, sizeof(struct Msg));
}


void handle_connect(int client_fd, struct Msg* msg){
    if(clients_connected == MAX_CLIENTS){
        write(client_fd, &(struct Msg){.type = SERVER_FULL}, sizeof(struct Msg));
        printf("Connection failed, server full\n");
        return;
    }

    if (check_username(msg) == -1){
        write(client_fd, &(struct Msg){.type = NICK_TAKEN}, sizeof(struct Msg));
        printf("Connection failed, username already exists\n");
        return;
    }

    int id;
    id = create_client(client_fd, msg);
    ...
    if(clients_connected % 2 == 0){
        ...
        client_waiting_id = id;
        write(client_fd, &(struct Msg){.type = WAITING_FOR_OPPONENT}, sizeof(struct Msg));
    }
...
void handle_move(int client_fd, struct Msg* msg){
    ...
    if (clients[msg->id]->game->board[msg->move_id - 1] != ' '){
        ...
        write(clients[players[turn % 2]]->fd, msg, sizeof(struct Msg));
    ...
    if (is_over(clients[msg->id]->game->board, symbol)){
        ...
        write(clients[players[turn % 2]]->fd, msg, sizeof(struct Msg));
        write(clients[players[(turn + 1) % 2]]->fd, msg, sizeof(struct Msg));
        return ;
    }
    ...
    if (turn + 1 == 9){
        ...
        msg->type = END;
        msg->winner = -1;
        write(clients[players[0]]->fd, msg, sizeof(struct Msg));
        write(clients[players[1]]->fd, msg, sizeof(struct Msg));
        return ;
    }
...
void handle_exit(){
    ...
    for(int i = 0; i < MAX_CLIENTS; i++){
        if(clients[i] != NULL){
            write(clients[i]->fd, &msg, sizeof(struct Msg));
            read(clients[i]->fd, &msg, sizeof(struct Msg));
            disconnect_player(i);
        }
